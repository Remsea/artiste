...
const entranceBoucing = (mybuilding) => {
    ...
    for (i = 0; i < mybuilding.children.length; i++) {
        ...
        showUpEtiquetteInfoConstant(mybuilding.children[i]);
        addClassBounce(mybuilding.children[i].firstElementChild, j);
        j--;
    }
}
...
const showUpEtiquetteInfoConstant = (element) => {
    element.querySelector('.etiquette-info').style.left = '46em';
}

// supprimer la class bouncing sur les etages
const removeClassBounce = (mybuilding) => {
    let i;
    for (i = 0; i < mybuilding.children.length; i++) { mybuilding.children[i].firstElementChild.classList.remove('bounce-in-top'); }
}
...
const updateValuesEtage = (element) => {
    ...
    let etiquetteInfo = parentcontainer.querySelector('.etiquette-info');
    let surfaceUtilise = parseInt(element.dataset.originalcapacity) - parseInt(element.dataset.capacity);
    let surfaceOriginal = element.dataset.originalcapacity;
    if (surfaceUtilise == surfaceOriginal) { element.querySelector('.empty').style.opacity = 0; } else { element.querySelector('.empty').style.opacity = 1; }
